...
async function salvarUsuario(i) {
  ...
  try {
    // 1️⃣ Pega SHA mais recente do arquivo
    await getData(URLS.users, "users"); // isso atualiza shaUsers

    // 2️⃣ Envia update com SHA atualizado
...
async function resetSenha(i) {
  ...
  try {
    // 1️⃣ Hash da senha com SHA-256
    const hashed = await hashPassword("123456");
    user.senha = hashed;

    // 2️⃣ Atualiza SHA mais recente do arquivo
    await getData(URLS.users, "users"); // atualiza shaUsers

    // 3️⃣ Envia atualização
    await updateData(URLS.users, usuarios, shaUsers);

    // 4️⃣ Recarrega lista e mostra mensagem
    await loadAll();
...
async function removerUsuario(i) {
  ...
  usuarios.splice(i, 1); // remove localmente

  try {
    // 1️⃣ Pega SHA mais recente do arquivo antes de atualizar
    await getData(URLS.users, "users"); // atualiza shaUsers
...
async function salvarNovoServico() {
  ...
  const novoServico = {
    id: servicos.length ? Math.max(...servicos.map(s => s.id)) + 1 : 1, // gera id único
    nome,
